...
class VideoPlayerViewModel: NSObject, ObservableObject {
  ...
  var explicitStartSeconds: Double?
  ...
  func setupPlayerItem(with url: URL) {
    ...
    statusObserver = playerItem?.observe(\.status, options: [.new, .initial]) {
      [weak self] item, _ in
      guard let self = self else { return }
    ...
    bufferingObserver = playerItem?.observe(\.loadedTimeRanges, options: [.new]) {
      [weak self] item, _ in
      guard let self = self else { return }
