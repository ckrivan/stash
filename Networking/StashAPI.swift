...
class StashAPI: ObservableObject {
  ...
  private func fetchPerformerMarkersLegacy(performerId: String, page: Int = 1) async throws
    -> [SceneMarker] {
  ...
  private func configureRequestWithAuth(_ request: inout URLRequest, referer: String? = nil) {
    ...
    print(
      "   API Key: \(apiKey.isEmpty ? "EMPTY!" : "[\(apiKey.prefix(10))...] (\(apiKey.count) chars)")"
    )
  ...
  func fetchScenes(
    ...
    do {
      ...
      do {
        ...
        print(
          "📦 Direct decoding failed, trying with GraphQLResponse wrapper: \(error.localizedDescription)"
        )

        // Try to decode with GraphQLResponse wrapper
        let scenesResponse = try JSONDecoder().decode(
          GraphQLResponse<ScenesResponseData>.self, from: data)
  ...
  private func tryFallbackSceneLoading(page: Int, sort: String, direction: String) async {
    do {
      ...
      print(
        "🔍 Fallback filtered out \(allSimpleScenes.count - filteredSimpleScenes.count) VR scenes")
  ...
  func fetchScenesExcludingVR(
    ...
    print(
      "🔄 StashAPI.fetchScenesExcludingVR called - page: \(page), sort: \(sort), direction: \(direction)"
    )
  ...
  func executeGraphQLQuery<T: Decodable>(
    _ query: String, variables: [String: Any]? = nil,
  ...
  func executeGraphQLQuery<T: Decodable>(
    query: String, variables: [String: Any]? = nil, completion: @escaping (Result<T, Error>) -> Void
  ) {
    Task {
      do {
        let response: T = try await performGraphQLRequest(query: query, variables: variables)
  ...
  func fetchPerformerScenes(
    ...
    print(
      "🔍 Fetching performer scenes (ID: \(performerId), page: \(page), sort: \(sort), direction: \(direction))"
    )
    ...
    var request = URLRequest(url: url)
    request.httpMethod = "POST"
    configureRequestWithAuth(
      &request, referer: "\(serverAddress)/performers/\(performerId)/scenes?sortby=date")
    request.httpBody = query.data(using: .utf8)
    ...
    do {
      ...
      if sceneCount > 0 {
        ...
        for i in 0..<checkCount {
          ...
          let containsPerformer = scene.performers.contains { $0.id == performerId }
          print(
            "📊 Scene \(i + 1) (\(scene.id)): \(scene.title ?? "Untitled") - Contains performer: \(containsPerformer)"
          )
      ...
      await MainActor.run {
        ...
        let verifiedScenes = scenesResponse.data.findScenes.scenes.filter { scene in
          scene.performers.contains { $0.id == performerId }
        }

        if verifiedScenes.count < scenesResponse.data.findScenes.scenes.count {
          print(
            "⚠️ Filtered out \(scenesResponse.data.findScenes.scenes.count - verifiedScenes.count) scenes that didn't include performer \(performerId)"
          )
        }
  ...
  func fetchTaggedScenes(
    tagId: String, page: Int = 1, perPage: Int = 40, sort: String = "date",
    direction: String = "DESC"
  ) async throws -> [StashScene] {
    ...
    request.setValue(
      "\(serverAddress)/scenes?c=(\"type\":\"tags\",\"value\":[\"\(tagId)\"],\"modifier\":\"INCLUDES\")&sortby=date",
  ...
  func getSystemStatus(completion: @escaping (Result<SystemStatus, Error>) -> Void) {
    ...
    ) { (result: Result<SystemStatusResponse, Error>) in
      switch result {
  ...
  func fetchStats() async throws -> StashStats {
    do {
      ...
      let response: GraphQLResponse<StatsDataResponse> = try await performGraphQLRequest(
        query: query)
  ...
  func fetchRandomScene(completion: @escaping (Result<StashScene, Error>) -> Void) {
    ...
    executeGraphQLQuery(
      query: query, variables: nil
    ) { (result: Result<SceneResponse, Error>) in
      switch result {
      case .success(let response):
  ...
  func fetchRandomPerformer(completion: @escaping (Result<StashScene.Performer, Error>) -> Void) {
    ...
    executeGraphQLQuery(
      query: query, variables: nil
    ) { (result: Result<PerformerResponse, Error>) in
      switch result {
      case .success(let response):
  ...
  func createTag(name: String) async throws -> StashScene.Tag {
    ...
    let response: TagCreateResponse = try await performGraphQLRequest(
      query: query, variables: variables)
    return response.tagCreate
  }

  func findTags(
    filter: TagFilter? = nil, completion: @escaping (Result<[StashScene.Tag], Error>) -> Void
  ) {
    let query = """
    ...
    executeGraphQLQuery(
      query: query, variables: variables
    ) { (result: Result<TagsResponse, Error>) in
      switch result {
      case .success(let response):
  ...
  func filterScenesByPreset(
    preset: FilterPreset, page: Int = 1, perPage: Int = 40, appendResults: Bool = false
  ) async {
    isLoading = true
    ...
    do {
      ...
      await MainActor.run {
        ...
        if appendResults {
          ...
          print(
            "✅ Loaded \(self.scenes.count) scenes for preset \(preset.rawValue) (filtered from \(allScenes.count) total)"
          )
        }
  ...
  func updateSceneTags(sceneID: String, tagIDs: [String]) async throws -> StashScene {
    ...
    let response: SceneUpdateResponse = try await performGraphQLRequest(
      query: query, variables: variables)
  ...
  func incrementSceneOCounter(sceneID: String, currentValue: Int) async throws -> StashScene {
    ...
    let response: SceneAddOResponse = try await performGraphQLRequest(
      query: query, variables: variables)
    ...
    if let updatedScene = try await fetchScene(byID: sceneID) {
      ...
      throw NSError(
        domain: "StashAPI", code: -1,
        userInfo: [
          NSLocalizedDescriptionKey: "Failed to fetch updated scene after incrementing o_counter"
        ])
    }
  }
  ...
  func fetchPerformers(
    ...
    completion: @escaping (Result<[StashScene.Performer], Error>) -> Void
  ) {
    isLoading = true
        ...
        do {
          ...
          await MainActor.run {
            if appendResults {
              ...
              print(
                "📊 PerformersAPI: Setting performers array with \(response.data.findPerformers.performers.count) performers"
              )
              self.performers = response.data.findPerformers.performers
              self.totalPerformerCount = response.data.findPerformers.count
              print(
                "📊 PerformersAPI: After setting, self.performers has \(self.performers.count) items"
              )
  ...
  func searchScenes(query: String, completion: @escaping (Result<[StashScene], Error>) -> Void) {
    ...
    executeGraphQLQuery(
      query: graphQLQuery, variables: variables
    ) { (result: Result<ScenesResponseData, Error>) in
      switch result {
      case .success(let response):
  ...
  func searchScenes(query: String, excludeVR: Bool = true) async throws -> [StashScene] {
    ...
    let scenesResponse: ScenesResponse = try await performGraphQLRequest(
      query: graphQLQuery, variables: variables)
    ...
    for (index, scene) in scenesResponse.data.findScenes.scenes.prefix(3).enumerated() {
      print(
        "  Scene \(index): ID=\(scene.id), Title='\(scene.title ?? "")', Details=\(scene.details != nil ? "yes" : "no")"
      )
    }
  ...
  func findScenes(filter: SceneFilterType, page: Int = 1, perPage: Int = 20) async throws -> (
    ...
    let response: FindScenesResponse = try await performGraphQLRequest(
      query: graphQLQuery, variables: variables)
    ...
    if let excludedTags = filter.excludedTags, !excludedTags.isEmpty {
      ...
      filteredScenes = filteredScenes.filter { scene in
        ...
        return tagsToExclude.isDisjoint(with: sceneTags)
          && tagsToExclude.isDisjoint(with: sceneTagIds)
      }
  ...
  ///   - page: Page number
  ///   - completion: Callback with result
  func fetchPerformerMarkers(
    performerId: String, page: Int = 1, completion: @escaping (Result<[SceneMarker], Error>) -> Void
  ) {
  ...
  ///   - completion: Callback with result
  func searchMarkers(
    query: String, page: Int = 1, perPage: Int = 2000, randomize: Bool = false,
    completion: @escaping (Result<[SceneMarker], Error>) -> Void
  ) {
    Task {
      do {
        let markers = try await searchMarkersCore(
          query: query, page: page, perPage: perPage, randomize: randomize)
        DispatchQueue.main.async {
          completion(.success(markers))
        }
      } catch {
  ...
  func searchMarkers(query: String, page: Int = 1, perPage: Int = 2000, randomize: Bool = false)
    async throws -> [SceneMarker] {
    return try await searchMarkersCore(
      query: query, page: page, perPage: perPage, randomize: randomize)
  }
  ...
  func searchMarkersWithCount(query: String, page: Int = 1, perPage: Int = 2000) async throws -> (
    [SceneMarker], Int
  ) {
  ...
  private func searchMarkersCore(
    query: String, page: Int = 1, perPage: Int = 2000, randomize: Bool = false
  ) async throws -> [SceneMarker] {
    ...
    let markersResponse: SceneMarkersResponse = try await performGraphQLRequest(
      query: graphQLQuery, variables: variables)
  ...
  private func searchMarkersCoreWithCount(query: String, page: Int = 1, perPage: Int = 2000)
    async throws -> ([SceneMarker], Int) {
    // Ensure the query is properly formatted
    let cleanQuery = query.trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    return (
      ...
      markersResponse.data.findSceneMarkers.count
    )
  }
  ...
  func searchMarkersByExactTagForShuffle(tagName: String, maxMarkers: Int = 2000) async throws
    ...
    let tagResponse: TagResponse = try await performGraphQLRequest(
      query: tagQuery, variables: tagVariables)

    // Find exact match
    guard
      let tag = tagResponse.data.findTags.tags.first(where: {
    ...
    let markerResponse: MarkerResponse = try await performGraphQLRequest(
      query: markerQuery, variables: markerVariables)
    let markers = markerResponse.data.findSceneMarkers.scene_markers

    print(
      "🎲 Shuffle search found \(markers.count) markers for tag '\(tagName)' (total available: \(markerResponse.data.findSceneMarkers.count))"
    )
    return markers
  }
  ...
  func searchMarkersBySuffix(suffix: String) async {
    ...
    do {
      ...
      let matchingTags = allTags.filter {
        $0.name.lowercased().hasSuffix("_\(suffix.lowercased())")
      }

      print(
        "🏷️ Found \(matchingTags.count) tags ending with '_\(suffix)': \(matchingTags.map { $0.name })"
      )
      ...
      for tag in matchingTags {
        ...
        print(
          "📊 Tag '\(tag.name)': Found \(tagMarkers.count) markers, \(uniqueMarkers.count) unique (Total: \(allSuffixMarkers.count))"
        )
      }
  ...
  func updateMarkersFromSearch(
    query: String, page: Int = 1, appendResults: Bool = false, perPage: Int = 2000
  ) async {
    isLoading = true
    ...
    do {
      ...
      await MainActor.run {
        ...
        print(
          "✅ Found \(newMarkers.count) markers matching '\(query)' on page \(page) (Searching with larger batch size of 500)"
        )
        ...
        if appendResults {
          ...
          print(
            "✅ Added \(uniqueNewMarkers.count) new unique markers (total now: \(self.markers.count))"
          )
  ...
  func createSceneMarker(
    ...
    executeGraphQLQuery(
      query: query, variables: variables
    ) { (result: Result<CreateMarkerResponse, Error>) in
      switch result {
      case .success(let response):
  ...
  func fetchMarkers(
    page: Int = 1, appendResults: Bool = false, performerId: String? = nil, perPage: Int = 500
  ) async {
    isLoading = true
    ...
    do {
      ...
      await MainActor.run {
        if appendResults {
          ...
          print(
            "✅ Set \(decodedResponse.data.findSceneMarkers.scene_markers.count) markers for tag \(tagId)"
          )
  ...
  private func fetchPerformerMarkersCore(performerId: String, page: Int = 1) async throws
    -> [SceneMarker] {
    ...
    if let httpResponse = response as? HTTPURLResponse {
      ...
      if httpResponse.statusCode >= 400 {
        ...
        if httpResponse.statusCode == 401 {
          ...
          print(
            "📤 Auth headers sent: ApiKey=\(request.value(forHTTPHeaderField: "ApiKey")?.prefix(5) ?? "nil"), Authorization=\(request.value(forHTTPHeaderField: "Authorization")?.prefix(12) ?? "nil")"
          )
        }
    ...
    do {
      ...
      if !markers.isEmpty {
        ...
        for i in 0..<displayCount {
          ...
          print(
            "🎯 Marker \(i + 1): ID=\(marker.id), Title=\(marker.title), Seconds=\(marker.seconds)")
        }
      }
