...
struct MarkersSearchResultsView: View {
  ...
  init(
    markers: [SceneMarker], totalCount: Int? = nil, onMarkerAppear: ((SceneMarker) -> Void)? = nil,
    onOpenTagSelector: (([SceneMarker.Tag]) -> Void)? = nil
  ) {
    ...
    VStack(spacing: 0) {
      ...
      if !markers.isEmpty {
        ...
        VStack(spacing: 16) {
          ...
          Text("This will combine markers from ALL specified tags")
            .font(.caption)
            .foregroundColor(.secondary)
        ...
        if !markers.isEmpty {
          ...
          if !appModel.searchQuery.isEmpty {
            ...
            appModel.startMarkerShuffle(
              forSearchQuery: appModel.searchQuery, displayedMarkers: markers)
          } else if let currentShuffleQuery = appModel.shuffleSearchQuery {
            print("🎲 Using existing shuffleSearchQuery: '\(currentShuffleQuery)'")
            appModel.startMarkerShuffle(
              forSearchQuery: currentShuffleQuery, displayedMarkers: markers)
          } else {
            ...
            if allSameTag {
              ...
              appModel.startMarkerShuffle(
                forTag: firstTag.id, tagName: firstTag.name, displayedMarkers: markers)
            } else {
...
struct MarkerRowWrapper: View {
  ...
  private func handleMarkerNavigation(_ selectedMarker: SceneMarker) {
    ...
    print(
      "⏱ MarkersSearchResultsView: Navigating to marker \(selectedMarker.title) at \(selectedMarker.seconds) seconds"
    )
  ...
  private func handleTagTap(_ tagName: String) {
    ...
    if primaryTag.name == tagName {
      ...
      appModel.startMarkerShuffle(
        forTag: primaryTag.id, tagName: primaryTag.name, displayedMarkers: [marker])
      ...
      appModel.startMarkerShuffle(
        forTag: foundTag.id, tagName: foundTag.name, displayedMarkers: [marker])
    } else {
