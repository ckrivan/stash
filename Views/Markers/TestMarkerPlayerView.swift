...
struct TestMarkerPlayerView: View {
    ...
    private func setupPlayer(for marker: SceneMarker, withURL specificURL: URL? = nil) {
        ...
        let sceneWithTimestampUrl = "\(baseServerURL)/scenes/\(sceneId)?t=\(Int(marker.seconds))\(apiKeySuffix)"
        let directMarkerStreamUrl = "\(baseServerURL)/scene/\(sceneId)/scene_marker/\(markerId)/stream\(querySuffix)"
        let markerPreviewUrl = marker.preview.isEmpty ? 
            "\(baseServerURL)/scene/\(sceneId)/preview\(querySuffix)" : 
            marker.preview + (marker.preview.contains("?") ? "&apikey=\(apiKey)" : (apiKey.isEmpty ? "" : "?apikey=\(apiKey)"))
    ...
    private func setupPlayer(withURL url: URL) {
        ...
        let timeObserver = player.addPeriodicTimeObserver(forInterval: CMTime(seconds: 0.5, preferredTimescale: 600), queue: .main) { time in
        ...
        if playerItem.accessibilityElements == nil {
            ...
            playerItem.accessibilityElements = (playerItem.accessibilityElements ?? []) + [statusObserver, timeObserver]
        }
    ...
    private func loadRandomMarker() async {
        ...
        do {
            ...
            guard let randomMarker = response.data.findSceneMarkers.scene_markers.first else {
                throw NSError(domain: "TestMarkerPlayer", code: 1, userInfo: [NSLocalizedDescriptionKey: "No random marker found"])
            }
    ...
    private func loadTestMarker() async {
        ...
        do {
            ...
            guard let firstMarker = response.data.findSceneMarkers.scene_markers.first else {
                throw NSError(domain: "TestMarkerPlayer", code: 1, userInfo: [NSLocalizedDescriptionKey: "No markers found"])
            }
    ...
    private func seekToRandomPosition() {
        guard let player = player, let marker = marker, let duration = player.currentItem?.duration.seconds, duration > 0 else { return }
