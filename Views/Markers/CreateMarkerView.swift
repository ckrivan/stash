...
struct CreateMarkerView: View {
  ...
  init(
    initialSeconds: String = "", sceneID: String = "", onMarkerCreated: @escaping () -> Void = {}
  ) {
    if !sceneID.isEmpty {
    ...
    let formattedSeconds =
      if let doubleValue = Double(initialSeconds) {
  ...
  private var isValid: Bool {
    print(
      "📍 Validation check - seconds: \(seconds), sceneID: \(sceneID), primaryTagId: \(primaryTagId)"
    )
    ...
    print(
      "📍 Validation details - hasSeconds: \(hasSeconds), hasSceneID: \(hasSceneID), hasTagID: \(hasTagID)"
    )
