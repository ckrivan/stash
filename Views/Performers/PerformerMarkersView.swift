...
struct PerformerMarkersView: View {
  ...
  private static let logger = Logger(
    subsystem: "com.ck.test.stash", category: "PerformerMarkersView")
  ...
  private func filteredMarkersForSelectedPerformer() -> [SceneMarker] {
    ...
    if let selectedTag = selectedTag {
      ...
      markers = markers.filter { marker in
        marker.primary_tag.id == selectedTag.id || marker.tags.contains { $0.id == selectedTag.id }
      }
              ...
              ForEach(markers) { marker in
                MarkerRow(
                  ...
                  onTitleTap: { marker in
                    ...
                    print(
                      "📱 PerformerMarkersView: Stopping all previews before navigating to full-screen"
                    )
                    GlobalVideoManager.shared.stopAllPreviews()
                    ...
                    DispatchQueue.main.async {
                      print(
                        "📱 PerformerMarkersView: Title tap detected for marker: \(marker.title)")
                      appModel.navigateToMarker(marker)
                    }
                  },
                  onTagTap: { tagName in
                    if let tag = appModel.api.markers.flatMap({ [$0.primary_tag] + $0.tags }).first(
                      where: { $0.name == tagName }) {
                      withAnimation {
  ...
  private func getMarkerCount(for performer: StashScene.Performer) async -> Int {
    ...
    do {
      Self.logger.debug(
        "Fetching marker count for performer: \(performer.name) (ID: \(performer.id))")
      ...
      Self.logger.debug(
        "Found \(response.data.findSceneMarkers.count) markers for performer \(performer.name)")
      return response.data.findSceneMarkers.count
    } catch {
      Self.logger.error(
        "Error fetching marker count for \(performer.name): \(error.localizedDescription)")
      return 0
    }
  }
  ...
  private func loadPerformerMarkers(performer: StashScene.Performer) async {
    ...
    do {
      ...
      let markers = try await appModel.api.fetchPerformerMarkers(
        performerId: performer.id, page: currentPage)
      ...
      Self.logger.info(
        "Successfully loaded \(markers.count) markers for performer \(performer.name)")
    } catch {
  ...
  private func loadMoreMarkers() async {
    ...
    Self.logger.info(
      "Loading more markers for performer: \(performer.name) (page \(currentPage + 1))")
    isLoadingMore = true
    currentPage += 1
    ...
    do {
      ...
      let newMarkers = try await appModel.api.fetchPerformerMarkers(
        performerId: performer.id, page: currentPage)
  ...
  private func sortMarkers() {
    ...
    case .sceneTitleAsc:
      appModel.api.markers.sort {
        ($0.scene.title ?? "").localizedCaseInsensitiveCompare($1.scene.title ?? "")
          == .orderedAscending
      }
    case .sceneTitleDesc:
      appModel.api.markers.sort {
  ...
  private func setHLSPreference(for marker: SceneMarker) {
    ...
    let hlsStreamURL =
      "\(baseServerURL)/scene/\(sceneId)/stream.m3u8?apikey=\(apiKey)&resolution=ORIGINAL&t=\(Int(markerSeconds))&_ts=\(currentTimestamp)"
