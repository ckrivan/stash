...
struct PerformerDetailView: View {
  ...
  var body: some View {
    ...
    .onDisappear {
      ...
      let isNavigatingToVideo =
        !appModel.navigationPath.isEmpty
            ...
            ForEach(markersList) { marker in
              ...
              .onAppear {
                ...
                NotificationCenter.default.post(
                  name: Notification.Name("ForceMarkerPreview\(marker.id)"), object: nil)
              }
              .frame(maxWidth: .infinity)
            }
          }
  ...
  private func loadScenes() async {
    print(
      "🔍 LOADSCENES - Starting load of scenes for performer: \(performer.name) (ID: \(performer.id))"
    )
    ...
    do {
      ...
      if let errors = response.errors, !errors.isEmpty {
        ...
        throw NSError(
          domain: "GraphQLError", code: 500, userInfo: [NSLocalizedDescriptionKey: errorMessages])
      }
  ...
  private func loadMarkers() async {
    print(
      "🔍 LOADMARKERS - Starting load of markers for performer: \(performer.name) (ID: \(performer.id))"
    )
    ...
    await appModel.api.fetchPerformerMarkers(
      performerId: performer.id, page: 1, appendResults: false)

    await MainActor.run {
      print(
        "✅ LOADMARKERS - Loaded \(appModel.api.markers.count) markers for performer: \(performer.name)"
      )
