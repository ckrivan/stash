...
struct PerformerTabView: View {
    ...
    var body: some View {
        ...
        .task {
            ...
            print("🚀 TASK: PerformerTabView appeared, selectedTab: \(selectedTab), performer: \(performer.name) (ID: \(performer.id))")
            ...
            if appModel.api.markers.isEmpty {
                ...
                VStack {
                    ...
                    ForEach(appModel.api.markers) { marker in
                                    ...
                                    HStack {
                                        ...
                                        if markerPreviewStates[marker.id, default: false] {
                                            Button(action: {
                                                ...
                                                Image(systemName: markerMuteStates[marker.id, default: true] ? "speaker.slash.fill" : "speaker.wave.2.fill")
                        ...
                        .animation(.spring(response: 0.3, dampingFraction: 0.7), value: markerPreviewStates[marker.id, default: false])
                        .onTapGesture {
                            // Set the HLS preference before navigating
                            setHLSPreference(for: marker)
                            appModel.navigateToMarker(marker)
    ...
    private func loadScenes() async {
        ...
        do {
            ...
            do {
                ...
                let backupResponse = try JSONDecoder().decode(DirectFindScenesResponse.self, from: backupData)
                
                await MainActor.run {
                    print("🔄 FALLBACK: Setting \(backupResponse.data.findScenes.scenes.count) scenes from backup")
    ...
    private func loadMarkers() async {
        ...
        do {
            ...
            print("✅ PERFORMER: Successfully loaded \(markers.count) markers for performer \(performer.name)")
    ...
    private func loadMoreMarkers() async {
        guard !isLoadingMore, hasMorePages else { 
            print("⚠️ Skipping loadMoreMarkers - isLoadingMore: \(isLoadingMore), hasMorePages: \(hasMorePages)")
            return 
        }
