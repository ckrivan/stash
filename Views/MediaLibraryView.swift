...
struct MediaLibraryView: View {
  ...
  @State private var lastValidatedTag: String?  // Track which tag was just validated
  ...
  var body: some View {
    mainView
      ...
      .onReceive(
        NotificationCenter.default.publisher(for: Notification.Name("ShowAdvancedFilters"))
      ) { _ in
        showingFilters = true
      }
  }
  ...
  private func tagSuggestionButton(tag: StashScene.Tag) -> some View {
    Button(action: {
      ...
      showingTagSuggestions = false
      searchText = tag.name
      forcedTagSelection = true  // Prevent suggestion loops
      Task {
  ...
  private var scenesContent: some View {
    ScrollView {
      if !searchedMarkers.isEmpty && searchScope == .markers && currentFilter == "search" {
        ...
        VStack {
          MarkersSearchResultsView(
            markers: searchedMarkers, totalCount: totalMarkerCount,
            onMarkerAppear: { _ in
      ...
      } else if searchedTag != nil && searchScope == .tags && currentFilter == "search"
        && !appModel.api.scenes.isEmpty {
        // Show tag search results
        TagSearchResultsView(
          scenes: appModel.api.scenes, tagName: searchedTag!.name, tagId: searchedTag!.id
        )
        .environmentObject(appModel)
  ...
  private func loadScenes() async {
    print(
      "📤 loadScenes called - currentFilter: \(currentFilter), searchScope: \(searchScope), markers count: \(searchedMarkers.count)"
    )
  ...
  private func loadMoreMarkers() async {
    guard hasMorePages && !isLoadingMore && searchScope == .markers && !searchText.isEmpty else {
      return
    }
    ...
    do {
      let newMarkers = try await appModel.api.searchMarkers(
        query: searchText, page: currentPage, perPage: 50)
      ...
      await MainActor.run {
        ...
        print(
          "✅ Added \(uniqueNewMarkers.count) new markers (total: \(searchedMarkers.count), hasMorePages: \(hasMorePages))"
        )
      }
    } catch {
    ...
    do {
      ...
      case .scenes:
        ...
        if !searchResults.isEmpty {
          print(
            "📊 First result: ID=\(searchResults[0].id), Title='\(searchResults[0].title ?? "no title")'"
          )
        }
        await MainActor.run {
        ...
        do {
          ...
          if let firstPerformer = performers.first {
            ...
            await appModel.api.fetchPerformerScenes(
              performerId: firstPerformer.id, page: 1, appendResults: false)
            ...
            await MainActor.run {
              print(
                "✅ Found \(appModel.api.scenes.count) scenes for performer: \(firstPerformer.name)")
              // Explicitly trigger UI update
              appModel.objectWillChange.send()
            }
          } else {
      ...
      case .markers:
        ...
        let tagPhrases = searchQuery.components(separatedBy: "+").map {
        ...
        if additionalTerms.isEmpty {
          ...
          if tagExists {
            ...
            let markers = try await appModel.api.searchMarkers(
              query: "#\(mainTerm)", page: 1, perPage: 20, randomize: true)
            let totalCount = tagCount  // Use the count from validation
            print(
              "📊 Found \(markers.count) markers using EXACT tag search for '\(mainTerm)' (total estimated: \(totalCount))"
            )
            ...
            // Check if user explicitly selected this tag from suggestions
            if forcedTagSelection {
              print("🏷️ Forcing tag selection despite validation failure: '\(mainTerm)'")
              forcedTagSelection = false  // Reset flag

              // Use fuzzy search instead of exact tag validation
              let markers = try await appModel.api.searchMarkers(
                query: mainTerm, page: 1, perPage: 20, randomize: true)
              let totalCount = markers.count
              print("📊 Found \(markers.count) markers using fuzzy search for '\(mainTerm)'")
              ...
              print(
                "⚠️ Tag '\(mainTerm)' does not exist in marker system - searching for similar tags")
          ...
          let mainMarkers = try await appModel.api.searchMarkers(
            query: "#\(mainTerm)", page: 1, perPage: 20, randomize: true)
          ...
          for additionalTerm in additionalTerms {
            let additionalMarkers = try await appModel.api.searchMarkers(
              query: "#\(additionalTerm)", page: 1, perPage: 20, randomize: true)
            let (_, additionalTagCount) = try await appModel.api.validateMarkerTag(
              tagName: additionalTerm)
            print(
              "📊 Additional term '\(additionalTerm)' (EXACT): \(additionalMarkers.count) markers for display"
            )
          ...
          print(
            "📊 Final combined results: \(finalMarkers.count) unique markers from \(1 + additionalTerms.count) tag searches"
          )
  ...
  private func updateMarkerResults(_ markers: [SceneMarker], _ count: Int) {
    ...
    print(
      "✅ Stored \(markers.count) marker results (total: \(count)) - filter: \(currentFilter), scope: \(searchScope), hasMorePages: \(hasMorePages)"
    )
    // Explicitly trigger UI update
    appModel.objectWillChange.send()
  }
