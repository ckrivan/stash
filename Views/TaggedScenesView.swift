...
struct TaggedScenesView: View {
  ...
  private func fetchTaggedScenes() async {
    ...
    request.setValue(
      "\(appModel.serverAddress)/scenes?c=(\"type\":\"tags\",\"value\":[\"\(tag.id)\"],\"modifier\":\"INCLUDES\")&sortby=date",
  ...
  private var shuffleButton: some View {
    Button(action: {
      ...
      appModel.startTagSceneShuffle(
        forTag: tag.id, tagName: tag.name, displayedScenes: appModel.api.scenes)
    }) {
      HStack(spacing: 4) {
