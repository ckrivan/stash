...
struct SceneRow: View {
  ...
  var body: some View {
        ...
        ZStack {
          ...
          VStack {
            HStack {
              ...
              Button {
                print(
                  "📱 SCENEROW: Random Jump button tapped for scene: \(scene.id), title: \(scene.title ?? "unknown")"
                )
            ...
            HStack(spacing: 12) {
              ...
              if let oCounter = scene.o_counter, oCounter > 0 {
                Button(action: {
                  ...
                  .padding(.vertical, 2)
                  .background(
                    isIncrementingOCounter ? Color.orange.opacity(0.3) : Color.orange.opacity(0.1)
                  )
                  .cornerRadius(6)
    ...
    .onReceive(
      NotificationCenter.default.publisher(for: Notification.Name("MainVideoPlayerStarted"))
    ) { _ in
  ...
  private func startPreview() {
    ...
    if let urlString = previewURLString, let url = URL(string: urlString) {
      ...
      let asset = AVURLAsset(
        url: url, options: ["AVURLAssetHTTPHeaderFieldsKey": request.allHTTPHeaderFields ?? [:]])
  ...
  private func incrementOCounter() async {
    ...
    do {
      ...
      async let apiCall = appModel.api.incrementSceneOCounter(
        sceneID: scene.id, currentValue: currentOCounter)
      ...
      print(
        "✅ SCENEROW: Successfully incremented o_counter for scene \(scene.id), from \(currentOCounter) to \(updatedScene.o_counter ?? 0)"
      )
