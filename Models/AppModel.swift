...
class AppModel: ObservableObject {
  ...
  init() {
    ...
    let savedAddress =
      UserDefaults.standard.string(forKey: "serverAddress") ?? "http://192.168.86.100:9999"
    ...
    Task(priority: .userInitiated) {
      ...
      do {
        ...
        await MainActor.run {
          ...
          api.isLoading = false
          print(
            "✅ NAVIGATION - Scene load completed with \(api.scenes.count) scenes via direct GraphQL"
          )
        }
      ...
      await MainActor.run {
        api.isLoading = false
        print(
          "✅ NAVIGATION - Scene load completed with \(api.scenes.count) scenes via standard API")
      }
    }
  }
  ...
  func navigateToMarker(_ marker: SceneMarker) {
    print(
      "🚀 NAVIGATION - Navigating to marker: \(marker.title) at \(marker.seconds) seconds in scene \(marker.scene.id)"
    )
    ...
    if !isMarkerShuffle && !isMarkerShuffleMode {
      print(
        "🎲 Scheduling auto-shuffle for tag: \(marker.primary_tag.name) (will start after video loads)"
      )
    ...
    Task {
      ...
      do {
        guard let fullScene = try await api.fetchScene(byID: marker.scene.id) else {
          throw NSError(
            domain: "AppModel", code: 404, userInfo: [NSLocalizedDescriptionKey: "Scene not found"])
        }
        ...
        print(
          "🎬 Starting navigation to scene with marker timestamp - should trigger immediate playback"
        )
  ...
  func forceCloseVideo(manualExit: Bool = true) {
    ...
    if manualExit {
      ...
      if !isMarkerShuffle && !isTagShuffle {
        ...
        print(
          "🎲 Skipping audio kill in forceCloseVideo - in shuffle mode (marker: \(isMarkerShuffle), tag: \(isTagShuffle))"
        )
      }
    }
  ...
  // Thread-safe flag to prevent concurrent audio cleanup
  private var isCleaningAudio = false
  private let audioCleanupQueue = DispatchQueue(label: "audio-cleanup", qos: .userInteractive)

  // New method specifically to kill all audio and players
  func killAllAudio() {
    // CRITICAL FIX: Prevent concurrent calls that cause deadlocks
    guard !isCleaningAudio else {
      print("🔇 Audio cleanup already in progress, skipping duplicate call")
      return
    }

    isCleaningAudio = true
    print("🔇 SAFE AUDIO CLEANUP: Killing all audio")
    ...
    // CRITICAL FIX: Don't block with wait() - this causes the deadlock!
    // Instead, schedule final cleanup to happen after all operations complete
    cleanupGroup.notify(queue: .main) {
      print("🔇 Final cleanup - all audio operations completed")
      GlobalVideoManager.shared.cleanupAllPlayers()

      // Reset the cleanup flag so future calls can proceed
      self.isCleaningAudio = false
    }
  }
  ...
  @Published var shuffleTagFilter: String?
  ...
  private func loadOptimizedMarkersForTag(
    tagId: String, tagName: String, displayedMarkers: [SceneMarker]
  ) async {
    ...
    while currentPage <= maxPages && allMarkers.count < maxMarkers {
      ...
      print(
        "📊 Page \(currentPage): Found \(newMarkers.count) markers, \(uniqueMarkers.count) unique (Total: \(allMarkers.count))"
      )
  ...
  private func fetchMoreMarkersForShuffle(searchQuery: String) async {
    ...
    do {
      ...
      let newMarkers = try await api.searchMarkers(
        query: searchQuery, page: randomPage, perPage: 50, randomize: true)
      ...
      if !newMarkers.isEmpty {
        await MainActor.run {
          ...
          print(
            "✅ Added \(uniqueMarkers.count) new markers to shuffle queue (total: \(self.markerShuffleQueue.count))"
          )
        }
      }
    } catch {
  ...
  func startMarkerShuffle(
    ...
    print(
      "🎲 Starting multi-tag shuffle with \(tagNames.count) tags: \(tagNames.joined(separator: ", "))"
    )
  ...
  func startMarkerShuffle(forSearchQuery query: String, displayedMarkers: [SceneMarker]) {
    print(
      "🎲 Starting marker shuffle for search: \(query) - using server-side random from ALL available markers"
    )
  ...
  func playNextServerSideMarker() async {
    print(
      "🎲 Fetching random marker from server for tags: \(shuffleTagNames.joined(separator: ", "))")
  ...
  private func fetchRandomMarkerForTag(_ tagName: String) async {
    print(
      "🎲 Fetching server-side random marker for tag: '\(tagName)' using shuffle-optimized search")

    do {
      // Use the shuffle-optimized search to get a random selection from ALL markers
      let shuffleMarkers = try await api.searchMarkersByExactTagForShuffle(
        tagName: tagName, maxMarkers: 100)
  ...
  private func loadAllMarkersForSearch(query: String) async {
    ...
    while currentPage <= maxPages {
      ...
      await api.updateMarkersFromSearch(
        query: query, page: currentPage, appendResults: false, perPage: 500)
      let newMarkers = api.markers
      ...
      if !newMarkers.isEmpty {
        ...
        for (index, marker) in sampleMarkers.enumerated() {
          ...
          print(
            "📊 Sample marker \(index + 1): '\(marker.title)' - Tag: '\(marker.primary_tag.name)' - Matches '\(query)': \(matchesQuery)"
          )
        }
      }
      ...
      print(
        "🔄 Page \(currentPage): Added \(uniqueNewMarkers.count) unique markers (total: \(allMarkers.count))"
      )
    ...
    await MainActor.run {
      ...
      print(
        "✅ Created shuffle queue with \(self.markerShuffleQueue.count) markers for search: '\(query)'"
      )
      ...
      if let firstMarker = self.markerShuffleQueue.first {
        ...
        if !self.navigationPath.isEmpty {
          ...
          UserDefaults.standard.set(
            true, forKey: "scene_\(firstMarker.scene.id)_isMarkerNavigation")
          UserDefaults.standard.set(
            Double(firstMarker.seconds), forKey: "scene_\(firstMarker.scene.id)_startTime")
          if let endSeconds = firstMarker.end_seconds {
            UserDefaults.standard.set(
              Double(endSeconds), forKey: "scene_\(firstMarker.scene.id)_endTime")
          }
  ...
  private func loadAllMarkersForMultiTagSearch(query: String) async {
    ...
    do {
      ...
      for additionalTerm in additionalTerms {
        let additionalMarkers = try await api.searchMarkers(
          query: "#\(additionalTerm)", page: 1, perPage: 500)
  ...
  private func loadAllMarkersForTag(tagId: String, tagName: String) async {
    ...
    do {
      ...
      let allMarkers = try await api.searchMarkersByExactTagForShuffle(
        tagName: tagName, maxMarkers: 1000)
  ...
  func nextMarkerInShuffle() -> SceneMarker? {
    ...
    print(
      "🎲 Next marker in shuffle: \(nextMarker.title) (random index \(randomIndex) of \(markerShuffleQueue.count))"
    )
    return nextMarker
  }
  ...
  private func nextMarkerWithBalancedTagRotation() -> SceneMarker? {
    ...
    print(
      "🎲 BALANCED ROTATION: Selected '\(selectedMarker.title)' from tag '\(selectedTag)' (\(tagMarkers.count) total, \(availableMarkers.count) available, weight: \(weight))"
    )
  ...
  func previousMarkerInShuffle() -> SceneMarker? {
    ...
    currentShuffleIndex =
      currentShuffleIndex > 0 ? currentShuffleIndex - 1 : markerShuffleQueue.count - 1
    let previousMarker = markerShuffleQueue[currentShuffleIndex]

    print(
      "🎲 Previous marker in shuffle: \(previousMarker.title) (index \(currentShuffleIndex) of \(markerShuffleQueue.count))"
    )
  ...
  func shuffleToNextMarker() {
    ...
    print(
      "🎲 Debug: Tag: \(nextMarker.primary_tag.name), Search Query: \(shuffleSearchQuery ?? "none")")
    if let query = shuffleSearchQuery {
      let matchesQuery =
        nextMarker.title.lowercased().contains(query.lowercased())
  ...
  @Published var shuffleTagId: String?
  ...
  private func loadAllScenesForTag(tagId: String, tagName: String) async {
    ...
    while currentPage <= maxPages {
      ...
      do {
        ...
        print(
          "🔄 Page \(currentPage): Added \(uniqueNewScenes.count) unique scenes (total: \(allScenes.count))"
        )
  ...
  private func previousSceneInShuffle() -> StashScene? {
    ...
    currentTagShuffleIndex =
      currentTagShuffleIndex > 0 ? currentTagShuffleIndex - 1 : tagSceneShuffleQueue.count - 1
    return tagSceneShuffleQueue[currentTagShuffleIndex]
  }
  ...
  func startMostPlayedShuffle(from allScenes: [StashScene]) {
    ...
    if let firstScene = mostPlayedShuffleQueue.first {
      print(
        "🎯 Starting playback with first most played scene: \(firstScene.title ?? "Untitled") (o_counter: \(firstScene.o_counter ?? 0))"
      )
  ...
  private func nextMostPlayedScene() -> StashScene? {
    ...
    currentMostPlayedShuffleIndex =
      (currentMostPlayedShuffleIndex + 1) % mostPlayedShuffleQueue.count
  ...
  private func previousMostPlayedScene() -> StashScene? {
    ...
    currentMostPlayedShuffleIndex =
      currentMostPlayedShuffleIndex > 0
      ? currentMostPlayedShuffleIndex - 1 : mostPlayedShuffleQueue.count - 1
    return mostPlayedShuffleQueue[currentMostPlayedShuffleIndex]
  }
  ...
  func shufflePerformerScenes(
    ...
    print(
      "🎭 Gender filtering - shouldDefaultToFemale: \(shouldDefaultToFemale), targetGender: \(targetGender ?? "any")"
    )
    print(
      "🎭 Current performer context: \(currentPerformer?.name ?? "none") (gender: \(currentPerformer?.gender ?? "unknown"))"
    )
    ...
    if let gender = targetGender {
      ...
      print(
        "🎭 Filtered to \(filteredScenes.count) scenes with \(gender.lowercased()) performers (from \(scenes.count) total)"
      )
    } else {
  ...
  private func shouldDefaultToFemalePerformers(currentPerformer: StashScene.Performer?) -> Bool {
    ...
    if let performer = performer {
      ...
      print(
        "🎭 Performer context detected: \(performer.name) (gender: \(performerGender ?? "unknown"))")
  ...
  private func performBasicSceneShuffle(fromScenes scenes: [StashScene]) {
    ...
    print(
      "🎲 Selected random scene: \(randomScene.title ?? "Untitled") from \(scenes.count) available scenes"
    )
